...
void analyze_hand(bool *straight, bool *flush, bool *four, bool *three,
        int num_in_rank[], int num_in_suit[], int *pairs);
...
void read_cards(int num_in_rank[], int num_in_suit[])
{
    bool card_exsists[NUM_RANKS][NUM_SUITS];
    ...
    while (cards_read < NUM_CARDS) 
    {
        ...
        else if (card_exsists[rank][suit])
            printf("Duplicate card; ignored\n");
        else {
            num_in_rank[rank]++;
            num_in_suit[suit]++;
            card_exsists[rank][suit] = true;
            cards_read++;
        }
    }
}
...
void print_result(bool straight, bool flush, bool four, bool three, int pairs)
{
    if (straight && flush)  printf("Staright flush");
